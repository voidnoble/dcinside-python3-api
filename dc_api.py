...
class API:
    ...
    async def board(self, board_id, num=-1, start_page=1, recommend=False, document_id_upper_limit=None, document_id_lower_limit=None, is_mini=False):
        page = start_page
        while num:
            if recommend:
    ...
    async def write_comment(self, board_id, document_id, contents="", dccon_id="", dccon_src="", parent_comment_id="", name="", password="", is_mini=False):
        url = "https://m.dcinside.com/board/{}/{}".format(
            board_id, document_id)
        async with self.session.get(url) as res:
            parsed = lxml.html.fromstring(await res.text())
    ...
    async def modify_document(self, board_id, document_id, title="", contents="", name="", password="", is_mini=False):
        if not password:
            url = "https://m.dcinside.com/write/{}/modify/{}".format(
                board_id, document_id)
            async with self.session.get(url) as res:
                return await self.__write_or_modify_document(board_id, title, contents, name, password, intermediate=await res.text(), intermediate_referer=url, document_id=document_id, is_mini=is_mini)
    ...
    async def remove_document(self, board_id, document_id, password="", is_mini=False):
        if not password:
            url = "https://m.dcinside.com/board/{}/{}".format(
                board_id, document_id)
            async with self.session.get(url) as res:
    ...
    async def write_document(self, board_id, title="", contents="", name="", password="", is_mini=False):
        return await self.__write_or_modify_document(board_id, title, contents, name, password, is_mini=is_mini)

    async def __write_or_modify_document(self, board_id, title="", contents="", name="", password="", intermediate=None, intermediate_referer=None, document_id=None, is_mini=False):
        if not intermediate:
            url = "https://m.dcinside.com/{write_path}/{board_id}".format(
                write_path='write' if not is_mini else 'writemini', board_id=board_id)
            async with self.session.get(url) as res:
                res_text = await res.text()
                if '페이지를 찾을 수 없습니다' in res_text:
                    return -404
                parsed = lxml.html.fromstring(res_text)
        else:
            if '페이지를 찾을 수 없습니다' in intermediate:
                return -404
            parsed = lxml.html.fromstring(intermediate)
            url = intermediate_referer
        first_url = url
        rand_code_node = parsed.xpath("//input[@name='code']")
        rand_code = rand_code_node[0].get(
            "value") if len(rand_code_node) else None
        user_id = parsed.xpath(
            "//input[@name='user_id']")[0].get("value") if not name else None
        mobile_key_node = parsed.xpath(
            "//input[@id='mobile_key']")
        mobile_key = mobile_key_node[0].get(
            "value") if len(mobile_key_node) else None
        hide_robot_node = parsed.xpath(
            "//input[@class='hide-robot']")
        hide_robot = hide_robot_node[0].get(
            "name") if len(hide_robot_node) else None
        csrf_token_node = parsed.xpath(
            "//meta[@name='csrf-token']")
        csrf_token = csrf_token_node[0].get(
            "content") if len(csrf_token_node) else None
        board_name_node = parsed.xpath(
            "//a[@class='gall-tit-lnk']")
        board_name = board_name_node[0].text.strip() if len(
            board_name_node) else None
        ...
        if rand_code:
            payload["rand_code"] = rand_code
        if is_mini:
            payload["is_mini"] = "1"
            payload["id"] = f'mi${payload["id"]}'
        async with self.session.post(url, headers=header, data=payload) as res:
            res = await res.text()
            res = json.loads(res)
        if not res["result"]:
        ...
        if '글등록은 30초이상이 지나야 가능' in res_text:
            print('[ERROR] 글등록은 30초이상이 지나야 가능')
        if 'refresh' in res_text:
            print('[INFO] 글등록 성공')
            # 응답으로부터 목록 URL 파싱
            list_url = res_text.replace(
                '<meta http-equiv="refresh" content="0; url=', '')
            list_url = list_url.replace('">', '')
            list_url = list_url.replace('"', '')

        # 목록 URL 페이지 파싱해서 방금 등록한 글 ID 조회
        if list_url:
            async with self.session.get(list_url, headers=header) as res:
                res_text = await res.text()
                parsed = lxml.html.fromstring(res_text)
            doc_id_nodes = parsed.xpath(
                f"//*[contains(text(),'{title}')]/ancestor::a[@class='lt']")
            doc_id = doc_id_nodes[0].get(
                "href") if len(doc_id_nodes) > 0 else 0
        # 글번호 값이 http url일 경우 번호만 추출
        if 'http' in doc_id:
            doc_id = doc_id.rsplit('/', 1).pop()
...
if version.major >= 3 and version.minor >= 8:
    class Test(unittest.IsolatedAsyncioTestCase):
        ...
        async def test_minor_write_mod_del_document_comment(self):
            board_id='stick'
            doc_id = await self.api.write_document(board_id=board_id, title="제목", contents="내용", name="닉네임", password="비밀번호", is_mini=True)
            doc = await self.api.document(board_id=board_id, document_id=doc_id)
            self.assertEqual(doc.contents, "내용")
            doc_id = await self.api.modify_document(board_id=board_id, document_id=doc_id, title="수정된 제목", contents="수정된 내용", name="수정된 닉네임", password="비밀번호", is_mini=True)
            doc = await self.api.document(board_id=board_id, document_id=doc_id)
            self.assertEqual(doc.contents, "수정된 내용")
            comm_id = await self.api.write_comment(board_id=board_id, document_id=doc_id, contents="댓글", name="닉네임", password="비밀번호")
            doc = await self.api.document(board_id=board_id, document_id=doc_id)
